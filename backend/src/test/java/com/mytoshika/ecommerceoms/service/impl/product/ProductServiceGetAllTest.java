...
class ProductServiceGetAllTest {
    ...
    void shouldGetAllProductsSuccessfully() {
        ...
        when(productRepo.searchProducts(
                eq(keyword),
                eq(minPrice),
                eq(maxPrice),
                eq(category),
        ...
        when(inventoryRepo.findByProduct(product))
                .thenReturn(Optional.of(inventory));

        when(mapper.map(product, ProductResponse.class))
        ...
        assertEquals(
                20,
                response.getContent().get(0).getAvailableQuantity()
        );

        verify(productRepo).searchProducts(
                eq(keyword),
                eq(minPrice),
                eq(maxPrice),
                eq(category),
    ...
    @Test
    void shouldThrowExceptionWhenPageSizeIsGreaterThanLimit() {
        ...
        // Assert
        assertEquals(
                "Page size must be between 1 to 100",
                exception.getMessage()
        );
    }
