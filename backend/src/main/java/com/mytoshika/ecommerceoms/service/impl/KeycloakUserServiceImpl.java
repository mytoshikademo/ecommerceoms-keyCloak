...
//        ----   Helper Method   -----
    ...
    private String createKeycloakUser(UsersResource usersResource, UserRepresentation user) {
        log.info("Sending create-user request to Keycloak for username: {}", user.getUsername());
        try (Response response = usersResource.create(user)) {
            log.info("Keycloak create-user response received. Status: {}", response.getStatus());
            if (response.getStatus() != 201) {
                String responseBody = response.readEntity(String.class);
                log.error("Keycloak user creation failed. Status: {}, Response: {}",
                        response.getStatus(), responseBody);
                throw new RuntimeException(
                        String.format(
                                "Failed to create user in Keycloak. Status: %d, Message: %s",
                                response.getStatus(),
                                responseBody
                        )
                );
            ...
            log.info("Keycloak user created successfully. User ID: {}", userId);
            return userId;
        } catch (Exception ex) {
            log.error("Exception while creating user in Keycloak", ex);
            throw ex;
        }
    }
