...
--changeset Abhishek:V7_add_product_category
ALTER TABLE product ADD COLUMN category VARCHAR(255);
UPDATE product SET category = CASE
                   WHEN name IN ('Wireless Mouse 3', 'Wireless Mouse 4') THEN 'Mouse'
                   WHEN name IN ('HP laptop', 'Mac book') THEN 'Laptop'
                   WHEN name = 'TV - Sansui' THEN 'TV'
                   ELSE 'Other'
    END;
ALTER TABLE product MODIFY COLUMN category VARCHAR(255) NOT NULL;
--rollback ALTER TABLE product DROP COLUMN category;